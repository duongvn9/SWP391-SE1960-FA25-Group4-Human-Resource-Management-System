...
<%--
    ...
    Expected attributes:
    - pagination: PaginationMetadata object with:
        - currentPage (int)
        - totalPages (int)
        - totalItems (long)
        - hasNext (boolean)
            ...
            <ul class="pagination justify-content-center">
                ...
                <li class="page-item ${!pagination.hasPrevious ? 'disabled' : ''}">
                    <c:choose>
                        <c:when test="${pagination.hasPrevious}">
                            <a class="page-link" href="javascript:void(0);" onclick="goToPage(${pagination.currentPage - 1})" aria-label="Previous">
                ...
                <c:choose>
                    ...
                    <c:otherwise>
                        ...
                        <c:forEach begin="${pagination.currentPage - 1}" end="${pagination.currentPage + 1}" var="pageNum">
                            <c:if test="${pageNum > 1 and pageNum < pagination.totalPages}">
                                <li class="page-item ${pagination.currentPage == pageNum ? 'active' : ''}">
                                    <c:choose>
                                        <c:when test="${pagination.currentPage == pageNum}">
                                            <span class="page-link">${pageNum}</span>
                        ...
                        <li class="page-item ${pagination.currentPage == pagination.totalPages ? 'active' : ''}">
                            <c:choose>
                                <c:when test="${pagination.currentPage == pagination.totalPages}">
                                    <span class="page-link">${pagination.totalPages}</span>
                                </c:when>
                                <c:otherwise>
                                    <a class="page-link" href="javascript:void(0);" onclick="goToPage(${pagination.totalPages})">
                ...
                <li class="page-item ${!pagination.hasNext ? 'disabled' : ''}">
                    <c:choose>
                        <c:when test="${pagination.hasNext}">
                            <a class="page-link" href="javascript:void(0);" onclick="goToPage(${pagination.currentPage + 1})" aria-label="Next">
