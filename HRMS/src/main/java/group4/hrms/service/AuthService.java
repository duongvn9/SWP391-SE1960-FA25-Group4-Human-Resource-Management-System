...
public class AuthService {
    ...
    public AuthResult authenticateLocal(String username, String password) {
        try {
            logger.info("Attempting local authentication for username: {}", username);
            ...
            if (!PasswordUtil.verifyPassword(password, credentials.getPasswordHash())) {
                ...
                // TODO: Increment failed attempts (nếu cần implement sau)
                return new AuthResult(false, "Invalid username or password");
            }
            ...
            if (!userOpt.isPresent()) {
                logger.error("User not found for account: {}", account.getId());
            ...
            logger.error("Error during local authentication for username: " + username, e);
    ...
    public AuthResult authenticateGoogle(String email, GoogleOAuthUtil.GoogleUserInfo googleUserInfo) {
        try {
            logger.info("Attempting Google authentication for email: {}", email);
            ...
            if (!identityOpt.isPresent()) {
                ...
                logger.info("Created new Google identity for account: {}", account.getId());
            } else {
            ...
            logger.error("Error during Google authentication for email: " + email, e);
            return new AuthResult(false, "Authentication error occurred");
        }
    }
